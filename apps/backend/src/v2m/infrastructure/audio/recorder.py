...
class AudioRecorder:
    ...
    def __init__(self, sample_rate: int = 16000, channels: int = 1, max_duration_sec: int = 600, device_index: Optional[int] = None):
        ...
        # corrección soporte para buffer multi-canal
        if self.channels > 1:
            ...
            with self._lock:
                ...
                if samples_to_write > 0:
                    ...
                    if self.channels > 1:
                        ...
                        # si indata tiene más columnas pero queremos mono tomamos canal 0
                        # si indata es (n, 1) lo aplanamos o cortamos
                        # indata[:samples_to_write, 0] da slice 1d
    ...
    def stop(self, save_path: Optional[Path] = None) -> np.ndarray:
        ...
        # corrección retornamos una copia para evitar corrupción de datos si se reinicia la grabación
        if self.channels > 1:
        ...
        if save_path:
            ...
            with wave.open(str(save_path), 'wb') as wf:
                ...
                # si channels > 1 el audio es 2d (samples, channels)
                # wave.writeframes espera bytes intercalados
                # tobytes() en array c-contiguous 2d da bytes row-major:
